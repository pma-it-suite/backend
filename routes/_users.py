...
def create_user():
    ...
    for field in required_fields:
        if field not in data:
            return jsonify(
                {"status": "error", "message": f"{field} is required"}), 400

    try:
        data["_id"] = str(ObjectId())  # Generate new ObjectId
        users_collection.insert_one(data)
        return jsonify(
            {"status": "OK", "message": f"User {data['username']} created successfully", "_id": data["_id"]}), 201
    except Exception as e:
        return jsonify(
            {"status": "error", "message": f"An error occurred: {str(e)}"}), 500
...
def update_user():
    ...
    if not username or not new_username:
        return jsonify(
            {"status": "error", "message": "Both username and new_username are required"}), 400
    ...
    if result.modified_count > 0:
        return jsonify(
            {"status": "OK", "message": f"User {username} updated successfully"}), 200
    else:
        return jsonify(
            {"status": "NO", "message": "User not found or not modified"}), 404
...
def delete_all_users():
    ...
    return jsonify(
        {"status": "OK", "message": f"{result.deleted_count} users deleted"})


@user_routes.route('/generate', methods=['POST'])
def generate_users():
    # default to generating 10 users
    ...
    users_collection.delete_many(
        {"username": {"$in": ["testadmin", "testuser"]}})
    ...
    for _ in range(count - len(special_users)
                   ):  # Adjust count for the special users
        user_id = str(ObjectId())
    ...
    return jsonify(
        {"status": "OK", "message": f"{len(result.inserted_ids)} users generated"})
